...
from custom_layers import fadein_layer, ConcatTable, minibatch_std_concat_layer, Flatten, pixelwise_norm_layer, \
    equalized_conv2d, equalized_deconv2d, equalized_linear
import copy
...
class Generator(nn.Module):
    ...
    def grow_network(self, resl):
        ...
        if resl >= 3 and resl <= 9:
            print(
                'growing network[{}x{} to {}x{}]. It may take few seconds...'.format(pow(2, resl - 1), pow(2, resl - 1),
                                                                                     pow(2, resl), pow(2, resl)))
