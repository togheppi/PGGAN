...
class ResnetBlock(torch.nn.Module):
    def __init__(self, num_filter, kernel_size=3, stride=1, padding=1, activation='relu', norm='batch', bias=True):
...
class Upsample2xBlock(torch.nn.Module):
    def __init__(self, input_size, output_size, upsample='deconv', activation='relu', norm='batch'):
...
class PixelNormBlock(torch.nn.Module):
    def __init__(self):
        super(PixelNormBlock, self).__init__()
        self.eps = 1e-8
    ...
    def forward(self, x):
        ...
        t = x.clone()
        t.data.fill_(0)
        norm = torch.sqrt(x.pow(2).sum(1, keepdim=True).expand_as(x).div(nf).add(self.eps))
